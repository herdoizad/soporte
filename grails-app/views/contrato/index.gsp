		...
		<div id="list-contrato" class="content scaffold-list" role="main">
			...
			<thead>
					<tr>
						...
						<th><g:message code="contrato.tipo.label" default="Tipo" /></th>
				...
				<g:each in="${contratoInstanceList}" status="i" var="contratoInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${contratoInstance.id}">${fieldValue(bean: contratoInstance, field: "fin")}</g:link></td>
					
						<td>${fieldValue(bean: contratoInstance, field: "path")}</td>
					
						<td>${fieldValue(bean: contratoInstance, field: "numero")}</td>
					
						<td>${fieldValue(bean: contratoInstance, field: "cliente")}</td>
					
						<td><g:formatDate date="${contratoInstance.inicio}" /></td>
					
						<td>${fieldValue(bean: contratoInstance, field: "tipo")}</td>
