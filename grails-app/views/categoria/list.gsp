...
<html>
    ...
    <!-- botones -->
            ...
            <div class="btn-group pull-right col-md-3">
                <div class="input-group">
                    ...
                    <span class="input-group-btn">
                        <g:link controller="categoria" action="list" class="btn btn-default btn-search">
        ...
        <table class="table table-condensed table-bordered table-striped table-hover">
            ...
            <tbody>
                <g:if test="${categoriaInstanceCount > 0}">
                    <g:each in="${categoriaInstanceList}" status="i" var="categoriaInstance">
                        <tr data-id="${categoriaInstance.id}">
        ...
        <elm:pagination total="${categoriaInstanceCount}" params="${params}"/>

        <script type="text/javascript">
            var id = null;
            function submitFormCategoria() {
                var $form = $("#frmCategoria");
                var $btn = $("#dlgCreateEditCategoria").find("#btnSave");
                if ($form.valid()) {
                    $btn.replaceWith(spinner);
                    openLoader("Guardando Categoria");
                    $.ajax({
                        type    : "POST",
            ...
            function deleteCategoria(itemId) {
                bootbox.dialog({
                    title   : "Alerta",
                    message : "<i class='fa fa-trash-o fa-3x pull-left text-danger text-shadow'></i><p>" +
                              "¿Está seguro que desea eliminar el Categoria seleccionado? Esta acción no se puede deshacer.</p>",
                    buttons : {
                        cancelar : {
                            label     : "Cancelar",
                        ...
                        eliminar : {
                            ...
                            callback  : function () {
                                openLoader("Eliminando Categoria");
                                $.ajax({
                                    type    : "POST",
